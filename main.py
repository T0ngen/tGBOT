...
async  def contacts(message: types.Message):
    await message.answer(f"Покупать товар у него: @mesudotech")
...
async def cmd_start(message: types.Message):
    await message.answer_sticker("CAACAgIAAxkBAAMoZBiwTyprcFIPb0O_AAFB8AYTvBUjAALmAAO2qz8LHUzDCmatilUvBA")
    await message.answer(f"{message.from_user.first_name}, добро пожаловать в магазин кроссовок! ", reply_markup=main)
    if message.from_user.id == int(os.getenv("ADMIN_ID")):
